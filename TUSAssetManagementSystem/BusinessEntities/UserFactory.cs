...
namespace BusinessEntities
{
    public static class UserFactory // Only concern of factory is to create objects. 
    {
        ...
        public static IUser GetUser(string name, string password, string userType, int userID)
        {
            if (user != null)  // ie is Factory is primed with an object. 
                return user;
            else
                return new User(name, password, userType, userID); // Factory coughs up a regular user (for production code) 
        }
