...
namespace ProductTracking
{
    public partial class formBookLibraryRoom : Form
    {
        ...
        public formBookLibraryRoom(IModel model, LibraryRoom selectedRoom, IUser user, DateTime date, DateTime startTime, DateTime endTime)
        {
            ...
            DateTime roundedStartTime = startTime.Date.AddHours(startTime.Hour);
            DateTime roundedEndTime = endTime.Date.AddHours(endTime.Hour);

            booking = new LibraryRoomBooking(0, user.UserID, room, date, roundedStartTime, roundedEndTime, false);
